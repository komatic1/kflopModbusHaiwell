...
{
	if (DoPCInt(PC_COMM_GET_DROS, TMP))
		return 1; // Var index and Cmd
...
{
	if (DoPCInt(PC_COMM_GET_MACHINE_COORDS, TMP))
		return 1; // Var index and Cmd
...
{
	if (DoPCInt(PC_COMM_GET_MISC_SETTINGS, TMP))
		return 1;
...
int GetToolTableIndexFromID(int ToolID, int *TIndex)
{
	...
	if (DoPCInt(PC_COMM_GET_TOOLTABLE_INDEX, ToolID))
		return 1;
...
{
	if (DoPCInt(PC_COMM_GET_TOOL_SLOT_ID, TMP))
		return 1;
...
{
	if (GetVars(5220, 1, TMP))
		return 1; // Download to persist TMP
...
{
	if (GetVars(5200 + FixtureIndex * 20 + Axis + 1, 1, TMP))
		return 1; // Download to persist TMP
...
int GetToolLength(int index, double *Length)
{
	...
	if (DoPCInt(PC_COMM_GET_TOOLTABLE_LENGTH, index))
		return 1; // Tool index and Cmd
...
{
	persist.UserData[PC_COMM_PERSIST + 2] = TMP; // persist offset (doubles)
	if (DoPCInt(PC_COMM_GET_TOOLTABLE_DIAMETER, index))
		return 1; // Tool index and Cmd
...
{
	persist.UserData[PC_COMM_PERSIST + 2] = TMP; // persist offset (doubles)
	if (DoPCInt(PC_COMM_GET_TOOLTABLE_OFFSETX, index))
		return 1; // Tool index and Cmd
...
{
	persist.UserData[PC_COMM_PERSIST + 2] = TMP; // persist offset (doubles)
	if (DoPCInt(PC_COMM_GET_TOOLTABLE_OFFSETY, index))
		return 1; // Tool index and Cmd
...
int GetEditControlDouble(double *d, int Var, int offset)
{
	int result;
	char s[80];
	if (GetEditControl(s, Var, offset))
		return 1;
